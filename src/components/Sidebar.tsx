...
function AgentsPanel() {
  const { agents, activeWorkspace, killAgent, startContinuation, stopContinuation } = useWorkspace()
  const list = Object.values(agents).filter(a => a.workspaceId === activeWorkspace?.id)

  if (list.length === 0) {
    return <div className="p-3 text-xs text-text-muted">No active agents. Spawn one using the bar below.</div>
  }
      ...
      {list.map(agent => (
        <AgentCard
          ...
          onToggleContinuation={() => {
            if (agent.continuationActive) stopContinuation(agent.ptyId)
            else if (agent.goal) startContinuation(agent.ptyId, agent.goal)
          }}
        />
      ))}
    </div>
...
function AgentCard({ agent, onKill, onToggleContinuation }: {
  agent: AgentSession
  onKill: () => void
  ...
  return (
    <div className="bg-bg rounded border border-border-primary p-2 flex flex-col gap-1">
      ...
      {agent.goal && (
        <div className="text-xs text-text-secondary truncate" title={agent.goal}>
          {agent.goal}
        </div>
      )}
      ...
      <div className="flex items-center justify-between mt-1">
        {agent.continuationActive ? (
          <div className="flex items-center gap-2">
            <span className="text-xs text-orange-400 animate-pulse-orange">
              Loop {agent.continuationIteration}/{agent.continuationMax}
            </span>
            <button
        ...
        <span className="text-xs text-text-muted font-mono">{agent.ptyId.slice(-6)}</span>
      </div>
    </div>
  )
}
...
function MemoryPanel() {
  const { memories, storeMemory, searchMemory, activeWorkspaceId } = useWorkspace()
    ...
    <div className="flex flex-col gap-2 p-2 h-full overflow-hidden">
      <div className="flex gap-1">
        <input
          ...
          className="flex-1 bg-bg border border-border-primary rounded px-2 py-1 text-xs outline-none focus:border-border-subtle"
        />
        <button onClick={handleSearch} className="px-2 py-1 bg-surface-hover rounded text-xs text-text-secondary hover:text-text-primary">
          🔍
        </button>
        <button onClick={() => setAdding(v => !v)} className="px-2 py-1 bg-surface-hover rounded text-xs text-text-secondary hover:text-text-primary">
          +
        </button>
      </div>
      ...
      {adding && (
        <div className="flex flex-col gap-1 bg-bg border border-border-primary rounded p-2">
          <input value={newKey} onChange={e => setNewKey(e.target.value)} placeholder="Key (e.g. 'auth strategy')" className="bg-transparent border border-border-primary rounded px-2 py-1 text-xs outline-none" />
          <textarea value={newContent} onChange={e => setNewContent(e.target.value)} placeholder="Content…" rows={3} className="bg-transparent border border-border-primary rounded px-2 py-1 text-xs outline-none resize-none" />
          <div className="flex gap-1 justify-end">
            <button onClick={() => setAdding(false)} className="px-2 py-1 text-xs text-text-muted hover:text-text-primary">Cancel</button>
            <button onClick={handleAdd} className="px-2 py-1 bg-surface-hover rounded text-xs text-text-primary hover:bg-border-primary">Save</button>
          </div>
        </div>
      )}
        ...
        {displayList.map(m => (
          <div key={m.id} className="bg-bg border border-border-primary rounded p-2">
            <div className="flex items-center justify-between mb-1">
              <span className="text-xs font-bold text-text-primary">{m.key}</span>
    ...
    <div className="flex flex-col gap-1 p-2 overflow-y-auto">
      {skills.map(skill => (
        <div key={skill.path} className="bg-bg border border-border-primary rounded p-2">
          <div className="text-xs font-bold text-text-primary mb-0.5">{skill.name}</div>
...
export default function Sidebar() {
  ...
  const { agents, activeWorkspace, memories } = useWorkspace()
  ...
  const tabs: Array<{ id: SidebarTab; label: string; badge?: number }> = [
    { id: 'agents', label: 'Agents', badge: agentCount || undefined },
      ...
      <div className="flex border-b border-border-primary">
        {tabs.map(tab => (
          <button
            ...
            className={`flex-1 py-2 text-xs relative transition-colors ${
              activeTab === tab.id
